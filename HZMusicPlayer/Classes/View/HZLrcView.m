...
@interface HZLrcView() <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, weak) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray *lrcLines;
@property (nonatomic, assign) int currentIndex;
@end

@implementation HZLrcView
...
- (void)setLrcname:(NSString *)lrcname
{
    ...
    for (NSString *lrcCmp in lrcCmps) {
        HZLrcLine *line = [[HZLrcLine alloc] init];
...
- (void)setCurrentTime:(NSTimeInterval)currentTime
{
    ...
    for (int idx = self.currentIndex + 1; idx<count; idx++) {
        HZLrcLine *currentLine = self.lrcLines[idx];
        // 当前模型的时间
        ...
        if (nextIdx < self.lrcLines.count) {
            HZLrcLine *nextLine = self.lrcLines[nextIdx];
...
{
    HZLrcCell *cell = [HZLrcCell cellWithTableView:tableView];
